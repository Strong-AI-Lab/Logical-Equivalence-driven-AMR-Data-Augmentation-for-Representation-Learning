...
def extract_logical_variables(conclusions, premises):
...
def infer_logical_expression(all_vn_phrases, all_negative_tags, all_compo_tags):
    ...
    # def implication(all_textual_vn_phrases, all_logical_expressions):
    #     cur_extended_logical_expression = list()
    #     cur_extended_textual_vn_phrases = list()
    #     tag = ["implication"]
    #
    #     for i in range(len(all_logical_expressions)):
    #         if len(all_logical_expressions[i]) == 2 and i == 0:
    #             cur_logical_expression = [[x, y] for x, y in all_logical_expressions[i]]
    #             cur_logical_expression[0][1] = bool(1-cur_logical_expression[0][1])
    #             if cur_logical_expression not in all_logical_expressions+cur_extended_logical_expression:
    #                 # all_logical_expressions.append(rever_cur_logical_expression)
    #                 # all_vn_phrases.append(all_vn_phrases[i][::-1])
    #                 cur_extended_logical_expression.append(cur_logical_expression)
    #                 cur_extended_textual_vn_phrases.append(all_textual_vn_phrases[i])
    #
    #     return cur_extended_logical_expression, cur_extended_textual_vn_phrases, tag
    ...
    while whether_continue:
        cur_rever_extended_logical_expression, cur_rever_extended_textual_vn_phrases, tag = reverse_logic(all_textual_vn_phrases+extended_textual_vn_phrases, all_logical_expressions+extended_logical_expression)
        ...
        cur_trans_extended_logical_expression, cur_trans_extended_textual_vn_phrases, tag = transfer_logic(all_textual_vn_phrases+extended_textual_vn_phrases, all_logical_expressions+extended_logical_expression)
        ...
        # cur_trans_extended_logical_expression, cur_trans_extended_textual_vn_phrases, tag = implication(all_textual_vn_phrases + extended_textual_vn_phrases, all_logical_expressions + extended_logical_expression)
        # extended_logical_expression += cur_trans_extended_logical_expression
        # extended_textual_vn_phrases += cur_trans_extended_textual_vn_phrases
        # tags += tag
    ...
    return all_logical_expressions, all_textual_vn_phrases, extended_logical_expression, extended_textual_vn_phrases, tags


# verbalize the logical expression into text
def logical_expression_to_text(logical_expression, vn_phrases, tag):
...
def get_negative_context_for_each_prem(prem_vn_phrases, prem_negative_tags, prem_compo_tags, each_premise, context):
    ...
    if negative_logical_expression is not None:
        negative_sentence = logical_expression_to_text(negative_logical_expression, negative_textual_vn_phrases, tag="") + " "
    else:
        negative_sentence = ""
...
def get_cur_negative_context(conclusion, premises, context, which_prem, none_premise=False):
    # context_sents = nltk.sent_tokenize(context)
    ...
    if none_premise:
        ...
        conc_vn_phrases, conc_negative_tags = extract_logical_variables(conclusion, premises)
        ...
        left_logical_expressions = [all_prem_vn_phrases, all_prem_negative_tags, all_prem_compo_tags]
        ...
        left_logical_expressions = [left_prem_vn_phrases, left_prem_negative_tags, left_prem_compo_tags]
...
def get_cur_all_extended_text(all_prem_vn_phrases, all_prem_negative_tags, all_prem_compo_tags):
    ...
    all_logical_expressions, all_textual_vn_phrases, extended_logical_expression, extended_textual_vn_phrases, tags = infer_logical_expression(spread_all_vn_phrases, spread_all_negative_tags, spread_all_compo_tags)
    ...
    #
    #     if whether_extend:
    if len(extended_logical_expression) != 0 and len(extended_textual_vn_phrases) != 0:
        extended_text = logical_expression_to_text(extended_logical_expression[0], extended_textual_vn_phrases[0], tags[0])
...
def save_all_negative_context(data_type=0):
    if data_type == 0:
        all_context_constituents_file = './output_result/lreasoner_synthetic_extended_sentences_list.csv'
        # all_options_constituents_file = './reclor-data/train_options_constituents_file.npy'
        negative_context_file = './output_result/lreasoner_synthetic_sentence_list_negative.csv'
        ...
        lr_file = './output_result/Synthetic_xfm_t5wtense_logical_equivalence_list.csv'
    elif data_type == 1:
        ...
        all_context_constituents_file = '../reclor-data/train_context_constituents_file_3.npy'
        all_options_constituents_file = '../reclor-data/train_options_constituents_file_3.npy'
        negative_context_file = '../reclor-data/train_negative_context_cp_v19_3.npy'
        # negative_context_file = '../reclor-data/test_negative_context_cp_v103.npy'
        extended_context_file_2 = '../reclor-data/train_extended_context_cp_v195_3.npy'
        extended_context_file_3 = '../reclor-data/train_extended_context_cp_v196_3.npy'
        lr_file = '../reclor-data/train_3.json'

    # conclusions_premises_constituents = np.load(all_context_constituents_file, allow_pickle=True)
    # answers_constituents = np.load(all_options_constituents_file, allow_pickle=True)

    conclusions_premises_constituents = pd.read_csv(all_context_constituents_file)

    contexts = list()
    labels = list()
    dataframe = pd.read_csv(lr_file)
    for index, row in dataframe.iterrows():
        contexts.append(row["Original_Sentence"])
    ...
    for index, row in conclusions_premises_constituents.iterrows():
        conclusions = []
        premises = [row['Parsed_Sentence']]
        ...
        if len(premises) == 0:
            context_sents = nltk.sent_tokenize(contexts[index])
        ...
        for which_premise, none_premise in zip(which_prem_list, none_premise_list):
            ...
            negative_context, delete, not_find, left_logical_expressions = get_cur_negative_context(conclusions, premises, contexts[index], which_premise, none_premise)
            if delete == 0:
                delete_num += 1
            if not_find == 0:
                not_find_num += 1
            ...
            cur_all_extended_context_2, cur_all_extended_context_3, max_length = get_cur_all_extended_text(all_prem_vn_phrases, all_prem_negative_tags, all_prem_compo_tags)
    ...
    df = pd.DataFrame(columns=['Original_Sentence', 'Parsed_Sentence', 'LReasoner_generated_positive_samples',
                               'Positive_samples_labels', 'LReasoner_generated_negative_samples', 'Negative_samples_labels'])

    for index, row in conclusions_premises_constituents.iterrows():
        df = df.append(
            {'Original_Sentence': row["Original_Sentence"], 'Parsed_Sentence': row['Parsed_Sentence'],
             'LReasoner_generated_positive_samples': row["LReasoner_generated_positive_samples"], "Positive_samples_labels": row["Positive_samples_labels"],
             'LReasoner_generated_negative_samples': all_negative_contexts[index][0], "Negative_samples_labels": 0},
            ignore_index=True)

    df.to_csv(negative_context_file)
...
if __name__ == '__main__':
    ...
    # save_all_negative_context(data_type=3)