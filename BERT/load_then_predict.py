...
def main():
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        total_prediction_list.extend(list(predictions.numpy()))
    ...
    data = []
    df_output = pd.DataFrame(data, columns=['sentence1', 'sentence2', 'label','prediction'])

    for index, row in df.iterrows():
        if row['label'] != total_prediction_list[index]:
            df_output = df_output.append(
                {'sentence1': row['sentence1'],
                 'sentence2': row['sentence2'],
                 'label': row['label'],
                 'prediction': total_prediction_list[index]}, ignore_index=True)

    df_output.to_csv("./our_model_prediction_result_v4.csv", index=None, encoding='utf8')
