...
def infer_logical_expression(all_vn_phrases, all_negative_tags, all_compo_tags):
    ...
    while whether_continue:
        ...
        if len(cur_rever_extended_logical_expression)== 0:
...
def get_cur_all_extended_text(conclusion, premises):
    ...
    extended_sentence = logical_expression_to_text(extended_logical_expression[0], extended_textual_vn_phrases[0])
    return all_extended_contexts, extended_sentence, \
...
def save_all_extended_context_replicated(data_type=0):
    if data_type == 0:
        all_context_constituents_file = './output_result/synthetic_sentence_constituents_file_list.csv'
        # all_context_constituents_file = './output_result/synthetic_sentence_constituents_file_train.npy'
        # all_context_constituents_file = './output_result/synthetic_sentence_constituents_file_train.npy'
        ...
        # extended_context_file = './output_result/lreasoner_synthetic_extended_sentences_train.npy'
        extended_context_file = './output_result/lreasoner_synthetic_extended_sentences_list.csv'
    ...
    df = pd.DataFrame(columns=['Original_Sentence', 'Parsed_Sentence','LReasoner_generated_positive_samples','Positive_samples_labels'])
    ...
    for index, row in conclusions_premises_constituents.iterrows():
        conclusions = []
        premises = np.array([row['Parsed_Sentence']])
        cur_all_extended_context, extended_sentence, logic_result  = get_cur_all_extended_text(conclusions, premises)
        # all_extended_logic.append(logic_result)

        # all_extended_contexts.append(cur_all_extended_context)
        # all_extended_sentences.append(extended_sentence)
        df = df.append(
            {'Original_Sentence': row["Original_Sentence"], 'Parsed_Sentence': row['Parsed_Sentence'], 'LReasoner_generated_positive_samples': extended_sentence, "Positive_samples_labels": 1},
            ignore_index=True)

    # np.save(extended_context_file, all_extended_contexts)
    # np.save(extended_context_file, all_extended_sentences)
    # np.save(extended_logic_file, all_extended_logic)
    df.to_csv(extended_context_file)

def save_all_extended_context(data_type=0):
    if data_type == 0:
        all_context_constituents_file = './reclor-data/train_context_constituents_file_more_cases.npy'
        # all_options_constituents_file = './reclor-data/train_options_constituents_file_3.npy'
        # extended_logic_file = '../reclor-data/train_extended_logic.npy'
        extended_context_file = './reclor-data/train_extended_context_constituents_file_more_cases.npy'
    ...
    for i in tqdm(range(len(conclusions_premises_constituents))):
        ...
        cur_all_extended_context, max_length, logic_result  = get_cur_all_extended_text(conclusions, premises)
        # all_extended_logic.append(logic_result)
...
def get_all_constituents(data_type=0):
    if data_type == 0:
        lr_file = './reclor-data/train_morecases.json'
        all_context_constituents_file = './reclor-data/train_context_constituents_file_more_cases.npy'
        # all_options_constituents_file = './reclor-data/train_options_constituents_file_3.npy'
...
def get_all_constituents_replicated(data_type=0):
    if data_type == 0:
        lr_file = './output_result/Synthetic_xfm_t5wtense_logical_equivalence_list.csv'
        # all_synthetic_sentences_constituents_file = './output_result/synthetic_sentence_constituents_file_train.npy'
        all_synthetic_sentences_constituents_file = './output_result/synthetic_sentence_constituents_file_list.csv'

    dataframe = pd.read_csv(lr_file)
    all_context_constituents = list()
    df_parser_result = pd.DataFrame(columns=['Original_Sentence','Parsed_Sentence'])
    for index, row in dataframe.iterrows():
        cur_context_constituents = list()
        sentences = nltk.sent_tokenize(row["Original_Sentence"])

        for each_sent in sentences:
            sent_result = predictor.predict(
                sentence=each_sent
            )
            df_parser_result = df_parser_result.append({'Original_Sentence': row["Original_Sentence"], 'Parsed_Sentence': sent_result['trees']},
                           ignore_index=True)
            # cur_context_constituents.append(sent_result['trees'])

        # all_context_constituents.append(cur_context_constituents)
    # np.save(all_synthetic_sentences_constituents_file, all_context_constituents)
    df_parser_result.to_csv(all_synthetic_sentences_constituents_file)
if __name__ == '__main__':
    # 先跑Allennlp，去抽取constituents，存下来， 跑完注释
    # get_all_constituents_replicated(data_type=0)
    # get_all_constituents(data_type=0)

    # 得到constituents储存文件之后，去得到extended conetxt
    # save_all_extended_context(data_type=0)
    save_all_extended_context_replicated(data_type=0)